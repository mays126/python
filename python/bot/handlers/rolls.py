...
async def roll_order_name(message: types.Message,state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        try:
            for i in menu_data:
                print(data['name'])
                if i[1] == 'sushi' and i[0] == data['name']:
            ...
            rolls.addNewRoll([some_data['id'],some_data['username'],some_data['userpass'],some_data['user_adress'],some_data['name'],some_data['price'],some_data['ingridients']],'delivery.db')
