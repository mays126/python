...
from keyboards import inline_delete_order_pizza_kb,inline_delete_order_burger_kb,inline_delete_order_roll_kb
...
Ингридиенты: {prod[3]}'''
        type = prod[1]
        if type == 'sushi':
            await message.reply(answer,reply_markup=inline_order_roll_kb)
            continue
        elif type == 'burger':
            await message.reply(answer,reply_markup=inline_order_burger_kb)
            continue
        else:
            await message.reply(answer,reply_markup=inline_order_pizza_kb)
            continue
...
Имя пользователя: {order[1]},
Адресс доставки: {order[3]},
Наименование товара: {order[4]},
Цена товара: {order[5]},
Ингридиенты: {order[6]}'''
